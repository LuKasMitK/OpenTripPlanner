...
public class AStar {
	...
	boolean iterate() {
		...
		Collection<Edge> edges = runState.options.arriveBy ? runState.u_vertex.getIncoming()
	...
	void runSearch(long abortTime) {
		...
		while (!runState.pq.empty()) { // Until the priority queue is empty:
			 ...
			 * This presents the negligible inefficiency that adjacent states
			 * are generated for a state which could be the last one you need to
	...
	/**
	 * @return number of visited nodes during dijkstra run
	 */
	public int getVisitedNodeCount() {
		return runState.nVisited;
	}

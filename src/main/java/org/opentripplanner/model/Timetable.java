...
public class Timetable implements Serializable {
	...
	public TripTimes getNextTrip(State s0, ServiceDay serviceDay, int stopIndex, boolean boarding) {
		...
		// Adjust for possible boarding time TODO: This should be included in
		// the trip and based on GTFS
		if (boarding) {
		...
		for (TripTimes tt : tripTimes) {
			...
			if (boarding) {
				...
				if (depTime < 0)
					continue; // negative values were previously used for
								// canceled trips/passed stops/skipped stops,
								// but
								// now its not sure if this check should be
								// still in place because there is a boolean
								// field
								// for canceled trips
		...
		for (FrequencyEntry freq : frequencyEntries) {
			...
			if (boarding) {
				int depTime = freq.nextDepartureTime(stopIndex, adjustedTime); // min
																				// transfer
																				// time
																				// included
																				// in
	 ...
	 *         criteria.
	 */
	public TripTimes getNextTrip(RoutingRequest req, int time, ServiceDay serviceDay, int stopIndex,
			boolean boarding) {
		/*
		 * Search at the state's time, but relative to midnight on the given
		 * service day.
		...
		// Adjust for possible boarding time TODO: This should be included in
		// the trip and based on GTFS
		//int time = serviceDay.secondsSinceMidnight(epocheTime);

		if (boarding) {
			time += req.getBoardTime(this.pattern.mode);
		} else {
			time -= req.getAlightTime(this.pattern.mode);
		}
		TripTimes bestTrip = null;
		// Stop currentStop = pattern.getStop(stopIndex);
		// Linear search through the timetable looking for the best departure.
		...
		for (TripTimes tt : tripTimes) {
			...
			if (!serviceDay.serviceRunning(tt.serviceCode))
				continue; // TODO merge into call on next line
			// if ( ! tt.tripAcceptable(s0, stopIndex)) continue; TODO
		...
		for (FrequencyEntry freq : frequencyEntries) {
			...
			if (!serviceDay.serviceRunning(tt.serviceCode))
				continue; // TODO merge into call on next line
			// TODO if ( ! tt.tripAcceptable(s0, stopIndex)) continue;
			// int adjustedTime = adjustTimeForTransfer(s0, currentStop,
			// tt.trip, boarding, serviceDay, time);
			// if (adjustedTime == -1) continue;
			LOG.debug("  running freq {}", freq);
			if (boarding) {
				int depTime = freq.nextDepartureTime(stopIndex, time); // min
																		// transfer
																		// time
																		// included
																		// in
				...
				int arvTime = freq.prevArrivalTime(stopIndex, time); // min
																		// transfer
																		// time
																		// included
																		// in
	 ...
	 * zero.
	 */
	private int adjustTimeForTransfer(State state, Stop currentStop, Trip trip, boolean boarding, ServiceDay serviceDay,
			int t0) {
		...
		int transferTime = transferTable.getTransferTime(state.getPreviousStop(), currentStop, state.getPreviousTrip(),
				trip, boarding);
	 ...
	 * Timetables. The {@link TimetableSnapshot} performs the protective copying
	 * of this Timetable. It is not done in this update method to avoid
	 * repeatedly cloning the same Timetable when several updates are applied to
	 * it at once. We assume here that all trips in a timetable are from the
	 * same feed, which should always be the case.
	 *
	 * @param tripUpdate
	 *            GTFS-RT trip update
	 * @param timeZone
	 *            time zone of trip update
	 * @param updateServiceDate
	 *            service date of trip update
	 * @return new copy of updated TripTimes after TripUpdate has been applied
	 *         on TripTimes of trip with the id specified in the trip descriptor
	 *         of the TripUpdate; null if something went wrong
	 */
	public TripTimes createUpdatedTripTimes(TripUpdate tripUpdate, TimeZone timeZone, ServiceDate updateServiceDate) {
		...
		// Though all timetables have the same trip ordering, some may have
		// extra trips due to
		...
		if (tripDescriptor.hasScheduleRelationship()
			...
			// The GTFS-RT reference specifies that StopTimeUpdates are sorted
			// by stop_sequence.
			...
			for (int i = 0; i < numStops; i++) {
				...
				if (match) {
					StopTimeUpdate.ScheduleRelationship scheduleRelationship = update.hasScheduleRelationship()
							? update.getScheduleRelationship() : StopTimeUpdate.ScheduleRelationship.SCHEDULED;
